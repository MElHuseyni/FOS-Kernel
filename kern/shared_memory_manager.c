...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	//panic("createSharedObject() is not implemented yet...!!");
	 //The calling environment
	 	//struct Frame_info*fram_info=NULL;
	...
	//	b) E_SHARED_MEM_EXISTS if the shared object already exists
	    struct Env* myenv = curenv;
	  	...
	  	{
	    struct Share *ptr_obj;
	  	struct Frame_Info*frame_info=NULL;
	  	int ID = 0;
	    size = ROUNDUP(size, PAGE_SIZE);
	  	ID=allocate_share_object(&ptr_obj);
	      if (ID == E_NO_SHARE)
	       return E_NO_SHARE;
	      else
	    {
	  	    ...
	  	    {
	  frame_info=NULL;
	  uint32 r= allocate_frame(&frame_info);
	  map_frame(myenv->env_page_directory, frame_info, virtual_address, PERM_USER|PERM_WRITEABLE|PERM_PRESENT);
	  add_frame_to_storage(shares[ID].framesStorage,frame_info,i);
	  virtual_address+=PAGE_SIZE;
	  		     i++;
	  	    }
          size*PAGE_SIZE;
	  	  shares[ID].ownerID=ownerID;
	  	  strcpy(shares[ID].name,shareName);
	  	  shares[ID].size=size;
	  	  shares[ID].references=1;
	  	  shares[ID].isWritable=isWritable;
	  	  return ID;
	  }
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
    ...
    {
      frame_info=NULL;
      frame_info=get_frame_from_storage(shares[ID].framesStorage,i);
      if(shares[ID].isWritable==1)
   map_frame(myenv->env_page_directory,frame_info,virtual_address,PERM_USER|PERM_WRITEABLE);
      else
   map_frame(myenv->env_page_directory,frame_info,virtual_address,PERM_USER &~ PERM_WRITEABLE);
      virtual_address+=PAGE_SIZE;
      i++;
