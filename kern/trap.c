...
{
    	struct Frame_Info*pointer;
	    uint32 size= env_page_ws_get_size(curenv);
	    allocate_frame(&pointer);
	    map_frame(curenv->env_page_directory,pointer,(void*)fault_va,PERM_USER|PERM_WRITEABLE);

	    int ret_page= pf_read_env_page(curenv, (void*)fault_va);
	    if (ret_page==E_PAGE_NOT_EXIST_IN_PF)
	        {
	    	fault_va=ROUNDDOWN(fault_va,PAGE_SIZE);
	        if(fault_va>=USER_HEAP_MAX&&fault_va<USTACKTOP)
	           {
	       uint32 ret=pf_add_empty_env_page (curenv,fault_va,1);//?????
	           }
		     else
		        {
	     ...
	     int i=0;
	    while(i<curenv->page_WS_max_size)
	    {
	    	uint32 Return = env_page_ws_is_entry_empty(curenv,i);
		    if (Return==1)
		      {
...
//Handle the page fault
  void page_fault_handler(struct Env * curenv, uint32 fault_va)
  {
   ...
   //panic("page_fault_handler() is not implemented yet...!!");
    ...
    {
     uint32 vic_address=env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
     uint32 permissions=pt_get_page_permissions(curenv,vic_address);
     if(permissions&PERM_MODIFIED)
     {
    	 struct Frame_Info*fram;
    	 uint32*ptr;
    	 fram=get_frame_info(curenv->env_page_directory,(void*)vic_address,&ptr);
    	 pf_update_env_page(curenv,(void*)vic_address,fram);
         unmap_frame(curenv->env_page_directory,(void*)vic_address);
     }
     else
     {
    	 unmap_frame(curenv->env_page_directory,(void*)vic_address);
