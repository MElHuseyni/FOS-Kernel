...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	//panic("create_page_table() is not implemented yet...!!");
       ...
       uint32 * ptr = kmalloc(PAGE_SIZE);
	   uint32 Physical = kheap_physical_address((unsigned int)ptr);
	   Physical |= (PERM_PRESENT | PERM_WRITEABLE |PERM_USER);
	  ptr_page_directory[PDX(virtual_address)]=Physical;
	   int i= 0;
		while(i<1024)
		{
		  ptr[i] = 0;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	   ...
	   uint32 max=virtual_address+size;
	   for(uint32 j=virtual_address;j<max;j+=PAGE_SIZE)
	   {
		   pf_remove_env_page(e,j);
	   }

	   for(uint32 j=virtual_address;j<max;j+=PAGE_SIZE)
	   {
	   int i=0;
	  while(i<e->page_WS_max_size)
		       {
         if(e->ptr_pageWorkingSet[i].virtual_address==j)
		       	{
		                env_page_ws_clear_entry(e,i);
		  	        	unmap_frame(e->env_page_directory,(void*)j);
		       		    break;
		        	}
		         i++;
		       }
	    }
	   for(int j=virtual_address;j<max;j+=PAGE_SIZE)
	   {
		   uint32*ptr=NULL;
		   get_page_table(e->env_page_directory,(void*)j,&ptr);
		   int emp=0;
		      	    if(ptr!=NULL)
		      	    {
