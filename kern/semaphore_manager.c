...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int s= get_semaphore_object_ID(ownerEnvID,semaphoreName);
			if (s== E_SEMAPHORE_NOT_EXISTS)
			{
				struct Semaphore  *allocatedObject;
				int x=allocate_semaphore_object(&allocatedObject);
				if (x!=E_NO_SEMAPHORE)
				{
					allocatedObject->ownerID=ownerEnvID;
			        strcpy(allocatedObject->name,semaphoreName);      //strcpy for string
					allocatedObject->value=initialValue;

					allocatedObject->empty=0;
	                return x;
				}
				else
				{
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int m = get_semaphore_object_ID(ownerEnvID,semaphoreName);
		//struct Semaphore  *allocatedObject;
		...
		if ( (semaphores[m].value)<0)
		{
		    ...
		    enqueue(&semaphores[m].env_queue,myenv);

			//enqueue(&myQueue, curenv)=ENV_BLOCKED;
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	int x = get_semaphore_object_ID(ownerEnvID,semaphoreName);
		...
		semaphores[x].value++;
		if (semaphores[x].value<=0)

		{

			 struct Env* queue= dequeue(&semaphores[x].env_queue);
			 queue->env_status=ENV_READY;
			 sched_insert_ready(queue);
		}
	// Steps:
	//	1) Get the Semaphore
